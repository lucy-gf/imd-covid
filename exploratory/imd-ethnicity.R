...
ethn$Ethnicity <- factor(ethn$Ethnicity, levels=c('Asian','Black','Mixed','White','Other'))

ggplot(ethn[ethn$Ethnicity %in% c('Asian','Black','Mixed','White','Other'),], 
       aes(x=Decile, y=Value, group=Ethnicity, colour=Ethnicity)) + 
  geom_line(lwd=1) + geom_point() + theme_minimal() + scale_color_brewer(palette='Set1') +
  ylab("Population in IMD decile (%)") + xlab("IMD decile") +
