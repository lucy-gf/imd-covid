...
LSOAs$IMD_rank <- rep.int(NA,nrow(LSOAs))
for(i in 1:nrow(LSOAs)){
  mini <- IMDLSOA[IMDLSOA$lsoa11cd == LSOAs$`LSOA Code`[i],]
  LSOAs$IMD_decile[i] <- mini$IMDDecil
  LSOAs$IMD_rank[i] <- mini$IMDRank
}
...
ggplot(rural_age, aes(x=Age, y=Proportion, group=IMD, color=IMD)) +
  ...
  ylab("Proportion") + xlab("Age group") + theme_minimal() + 
  theme(panel.background = element_rect(fill = "white",color=NA),
        ...
        axis.text.y = element_text(color=1)) + facet_grid(.~rural) +
...
ggplot(data, aes(x=imd, y=median, group=urban, col=urban)) + 
  geom_line(lwd=0.8) + theme_minimal() +
  geom_point(size=2) + scale_color_brewer(palette='Set1') + ylim(c(30,55)) +
  ylab("Median age") + xlab("IMD decile") + labs(color='Geography') + 
